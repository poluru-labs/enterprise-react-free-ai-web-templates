export function BarChart({ items = [], maxValue, unit = '' }) {
  ...
  return (
    <div className="gov-hbar-list" role="list">
      {items.map((item) => {
        const width = `${Math.max(5, (item.value / peak) * 100)}%`;
        return (
          <div className="gov-hbar" key={item.name} role="listitem">
            <div className="gov-hbar-meta">
              <span>{item.name}</span>
              <strong>
            ...
            <div className="gov-hbar-track">
              <div
                className="gov-hbar-fill"
                style={{ width, background: item.color || 'var(--gov-brand-600)' }}
              />
            </div>
          </div>
        );
    ...
    <div className="gov-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Grouped bar chart">
        {[0.25, 0.5, 0.75, 1].map((tick) => {
          ...
          return (
            <line key={tick} x1={padX} x2={width - padX} y1={y} y2={y} className="gov-chart-grid" />
          );
        })}
      ...
      <div className="gov-chart-labels">
        {labels.map((label) => (
