...
const navItems = [
  { label: 'Overview', icon: 'bi-grid-1x2-fill' },
  { label: 'Models', icon: 'bi-cpu' },
  { label: 'Prompts', icon: 'bi-chat-square-text' },
  { label: 'Evaluations', icon: 'bi-clipboard-check' },
  { label: 'Costs', icon: 'bi-bar-chart-line' },
];
...
export default function App() {
  ...
  const filteredModels = useMemo(
    () => models.filter((model) => model.name.toLowerCase().includes(query.toLowerCase()) || model.owner.toLowerCase().includes(query.toLowerCase())),
    [query]
  );

  const refreshData = () => setLastSynced('just now');
        ...
        <div className="llm-shell">
          <header className="llm-header">
            ...
            <div className="llm-header-actions">
              ...
              <Button variant="tertiary" size="sm" icon="arrow-clockwise" onClick={refreshData}>Refresh</Button>
              <button className="llm-icon-button" aria-label="Notifications"><i className="bi bi-bell" /></button>
          ...
          <div className="llm-body">
            <aside className="llm-sidebar">
              <nav aria-label="Main navigation">
                ...
                {navItems.map((item) => (
                  <button key={item.label} className={`llm-nav-item ${activeNav === item.label ? 'is-active' : ''}`} onClick={() => setActiveNav(item.label)}>
              ...
              <div className="llm-sidebar-bottom">
                <button className="llm-nav-item"><i className="bi bi-gear" /><span>Settings</span></button>
                <div className="llm-help"><i className="bi bi-life-preserver" /><span>Need help?</span><button aria-label="Open help"><i className="bi bi-arrow-up-right" /></button></div>
              </div>
            </aside>
            <main className="llm-main">
              <div className="llm-page-heading">
                <div><p className="llm-eyebrow">Workspace / {activeNav}</p><h1>Good morning, Subrahmanyam</h1><p className="llm-muted">Here is what is happening across your language model stack.</p></div>
                <Button variant="primary" icon="plus">Deploy model</Button>
              </div>
              ...
              <div className="llm-content-grid">
                <Card padded={false}>
                  <div className="llm-card-heading"><div><h2>Model health</h2><p>Performance across production models</p></div><Button variant="tertiary" size="sm" iconTrailing="arrow-up-right">View details</Button></div>
                  <div className="llm-table-wrap"><table className="llm-table"><thead><tr><th>Model</th><th>Requests</th><th>Latency</th><th>Success rate</th><th>Status</th></tr></thead><tbody>{filteredModels.map((model) => <tr key={model.name}><td><div className="llm-model-cell"><span className="llm-model-icon"><i className="bi bi-cpu" /></span><div><strong>{model.name}</strong><small>{model.provider} · {model.owner}</small></div></div></td><td>{model.calls}</td><td>{model.latency}</td><td><div className="llm-progress-cell"><ProgressBar value={model.success} max={100} /><span>{model.success}%</span></div></td><td><Badge variant={model.status === 'Healthy' ? 'success' : 'warning'} soft>{model.status}</Badge></td></tr>)}</tbody></table></div>
                </Card>
                <Card padded={false}>
                  <div className="llm-card-heading"><div><h2>Recent activity</h2><p>Latest workspace events</p></div><button className="llm-more-button" aria-label="More activity"><i className="bi bi-three-dots" /></button></div>
                  <div className="llm-activity-list">{activities.map((activity) => <div className="llm-activity" key={activity.title}><span className={`llm-activity-icon ${activity.tone}`}><i className={`bi ${activity.icon}`} /></span><div><strong>{activity.title}</strong><p>{activity.detail}</p></div><time>{activity.time}</time></div>)}</div>
                  <div className="llm-card-footer"><Button variant="tertiary" size="sm" iconTrailing="arrow-right">View all activity</Button></div>
                </Card>
              </div>
            </main>
          </div>
