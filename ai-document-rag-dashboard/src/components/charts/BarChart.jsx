export function BarChart({ items = [], maxValue, unit = '' }) {
  ...
  return (
    <div className="rag-hbar-list" role="list">
      {items.map((item) => {
        const width = `${Math.max(5, (item.value / peak) * 100)}%`;
        return (
          <div className="rag-hbar" key={item.name} role="listitem">
            <div className="rag-hbar-meta">
              <span>{item.name}</span>
              <strong>
            ...
            <div className="rag-hbar-track">
              <div
                className="rag-hbar-fill"
                style={{ width, background: item.color || 'var(--rag-brand)' }}
              />
            </div>
          </div>
        );
