...
export function AreaChart({ labels = [], series = [], height = 220 }) {
  ...
  return (
    <div className="kiln-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Loss trend">
        {[0.25, 0.5, 0.75, 1].map((tick) => {
          const y = padY + (height - padY * 2) * (1 - tick);
          return (
            <line key={tick} x1={padX} x2={width - padX} y1={y} y2={y} className="kiln-chart-grid" />
          );
        })}
        {fillSeries ? (
      ...
      <div className="kiln-chart-labels">
        {labels.map((label) => (
          <span key={label}>{label}</span>
        ))}
      </div>
      <div className="kiln-chart-legend">
        {series.map((item) => (
          <span key={item.name}>
