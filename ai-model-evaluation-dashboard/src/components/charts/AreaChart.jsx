...
export function AreaChart({ labels = [], series = [], height = 220 }) {
  ...
  return (
    <div className="prism-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Score trend">
        {[0.25, 0.5, 0.75, 1].map((tick) => {
          const y = padY + (height - padY * 2) * (1 - tick);
          return (
            <line key={tick} x1={padX} x2={width - padX} y1={y} y2={y} className="prism-chart-grid" />
          );
        })}
        {fillSeries ? (
          <path d={areaPath(fillSeries.values, width, height, padX, padY, max)} fill={fillSeries.color} opacity="0.14" />
        ) : null}
        {series.map((item) => (
          <path
            key={item.name}
      ...
      <div className="prism-chart-labels">
        {labels.map((label) => (
          <span key={label}>{label}</span>
        ))}
      </div>
      <div className="prism-chart-legend">
        {series.map((item) => (
          <span key={item.name}>
