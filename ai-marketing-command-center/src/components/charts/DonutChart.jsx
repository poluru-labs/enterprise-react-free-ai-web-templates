...
export function DonutChart({ items = [], centerLabel = 'Total', centerValue }) {
  ...
  return (
    <div className="mq-donut">
      <svg viewBox="0 0 160 160" role="img" aria-label={centerLabel}>
        <circle cx="80" cy="80" r={radius} className="mq-donut-track" />
        {slices.map((slice) => (
          <path
            key={slice.name}
        ...
        <text x="80" y="76" textAnchor="middle" className="mq-donut-value">
          {centerValue ?? total}
        </text>
        <text x="80" y="96" textAnchor="middle" className="mq-donut-label">
          {centerLabel}
        </text>
      </svg>
      <ul className="mq-donut-legend">
        {items.map((item) => (
          <li key={item.name}>
