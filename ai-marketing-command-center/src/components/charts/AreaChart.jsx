    ...
    <div className="mq-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Volume trend">
        {[0.25, 0.5, 0.75, 1].map((tick) => {
          ...
          return (
            <line key={tick} x1={padX} x2={width - padX} y1={y} y2={y} className="mq-chart-grid" />
          );
        })}
        {fillSeries ? (
          <path d={areaPath(fillSeries.values, width, height, padX, padY, max)} fill={fillSeries.color} opacity="0.18" />
        ) : null}
        {series.map((item) => (
          <path
            key={item.name}
      ...
      <div className="mq-chart-labels">
        {labels.map((label) => (
          <span key={label}>{label}</span>
        ))}
      </div>
      <div className="mq-chart-legend">
        {series.map((item) => (
          <span key={item.name}>
