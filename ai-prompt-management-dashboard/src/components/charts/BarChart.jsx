export function BarChart({ items = [], maxValue, unit = '' }) {
  ...
  return (
    <div className="pmt-hbar-list" role="list">
      {items.map((item) => {
        const width = `${Math.max(5, (item.value / peak) * 100)}%`;
        return (
          <div className="pmt-hbar" key={item.name} role="listitem">
            <div className="pmt-hbar-meta">
              <span>{item.name}</span>
              <strong>
            ...
            <div className="pmt-hbar-track">
              <div
                className="pmt-hbar-fill"
                style={{ width, background: item.color || 'var(--pmt-brand)' }}
              />
            </div>
          </div>
        );
...
export function GroupedBarChart({ labels = [], series = [], height = 220 }) {
  ...
  return (
    <div className="pmt-chart">
      ...
      <div className="pmt-chart-labels">
        {labels.map((label) => (
          <span key={label}>{label}</span>
        ))}
      </div>
      <div className="pmt-chart-legend">
        {series.map((item) => (
          <span key={item.name}>
