  ...
  return (
    <div className="cd-donut">
      <svg viewBox="0 0 160 160" role="img" aria-label={centerLabel}>
        <circle cx="80" cy="80" r={radius} className="cd-donut-track" />
        {slices.map((slice) => (
          <path
            key={slice.name}
        ...
        <text x="80" y="76" textAnchor="middle" className="cd-donut-value">
          {centerValue ?? total}
        </text>
        <text x="80" y="96" textAnchor="middle" className="cd-donut-label">
          {centerLabel}
        </text>
      </svg>
      <ul className="cd-donut-legend">
        {items.map((item) => (
          <li key={item.name}>
