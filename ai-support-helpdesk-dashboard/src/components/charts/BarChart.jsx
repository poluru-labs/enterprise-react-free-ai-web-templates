export function BarChart({ items = [], maxValue, unit = '' }) {
  ...
  return (
    <div className="desk-hbar-list" role="list">
      {items.map((item) => {
        const width = `${Math.max(5, (item.value / peak) * 100)}%`;
        return (
          <div className="desk-hbar" key={item.name} role="listitem">
            <div className="desk-hbar-meta">
              <span>{item.name}</span>
              <strong>
            ...
            <div className="desk-hbar-track">
              <div
                className="desk-hbar-fill"
                style={{ width, background: item.color || 'var(--desk-brand)' }}
              />
            </div>
          </div>
        );
    ...
    <div className="desk-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Grouped bar chart">
        {[0.25, 0.5, 0.75, 1].map((tick) => {
          ...
          return (
            <line key={tick} x1={padX} x2={width - padX} y1={y} y2={y} className="desk-chart-grid" />
          );
        })}
      ...
      <div className="desk-chart-labels">
        {labels.map((label) => (
