...
export function StatCard({
  ...
  return (
    <article className={`desk-stat-card tone-${tone}`}>
      <div className="desk-stat-top">
        <span
          className="desk-stat-icon"
      ...
      <p className="desk-stat-label">{label}</p>
      <p className="desk-stat-value">{value}</p>
      <div className="desk-stat-foot">
        {trendValue ? (
          <span className={`desk-stat-trend ${trendClass}`}>
            <i className={`bi ${trendIcon}`} aria-hidden="true" />
            {trendValue}
          </span>
        ) : null}
        {hint ? <span className="desk-stat-hint">{hint}</span> : null}
      </div>
    </article>
  );
}
