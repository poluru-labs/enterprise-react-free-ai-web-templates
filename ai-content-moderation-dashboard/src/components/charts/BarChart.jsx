export function BarChart({ items = [], maxValue, unit = '' }) {
  ...
  return (
    <div className="cmb-hbar-list" role="list">
      {items.map((item) => {
        const width = `${Math.max(5, (item.value / peak) * 100)}%`;
        return (
          <div className="cmb-hbar" key={item.name} role="listitem">
            <div className="cmb-hbar-meta">
              <span>{item.name}</span>
              <strong>
            ...
            <div className="cmb-hbar-track">
              <div
                className="cmb-hbar-fill"
                style={{ width, background: item.color || 'var(--cmb-brand)' }}
              />
            </div>
          </div>
        );
    ...
    <div className="cmb-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Grouped bar chart">
        {[0.25, 0.5, 0.75, 1].map((tick) => {
          ...
          return (
            <line key={tick} x1={padX} x2={width - padX} y1={y} y2={y} className="cmb-chart-grid" />
          );
        })}
      ...
      <div className="cmb-chart-labels">
        {labels.map((label) => (
          <span key={label}>{label}</span>
        ))}
      </div>
      <div className="cmb-chart-legend">
        {series.map((item) => (
          <span key={item.name}>
